...
# Very noisy script because it executes sc.exe everytime it looks at a service

# Powershell script looks for the "ChangeConfig" permission on a service
# If the user logged in has this permission then they can privilege escalate to the user running the service
# Used Get-ModifiableService from PowerUp and reverse engineered the process.
#Reference: https://rohnspowershellblog.wordpress.com/2013/03/19/viewing-service-acls/
...
function Get-ServiceAcl {
    ...
    # This is very noisy...  Get-ModifiableService from PowerUp uses the Windows API
...
ForEach ($s in $services) {
    $acl = Get-ServiceAcl -Name $s.Name
    ForEach ($access in $acl.Access) {
        if (($access.ServiceRights -like "*ChangeConfig*") -and ($access.IdentityReference -ne "BUILTIN\Administrators") -and ($access.IdentityReference -ne "NT AUTHORITY\SYSTEM") -and ($access.IdentityReference -ne "NT SERVICE\TrustedInstaller")) {
            "-----------------------------------"
            $acl.Name
            $access
        }
